  ...
  const { data: searchResults } = useQuery({
    ...
    queryFn: async () => {
      if (debouncedSearch.length > 0) {
        return await searchAttendee({ attendanceBookId, name: debouncedSearch }).then((res) => res.data);
      }
    },
  ...
  return (
    <div className="flex flex-col gap-2 max-h-[300px] overflow-y-auto">
      ...
      {searchResults?.content?.length ? (
        ...
        debouncedSearch.length > 0 && <p className="text-gray-500 text-center">검색 결과가 없습니다.</p>
      )}
    </div>
  );
};