...
효율성을 고려해 React로 마이그레이션을 하게 되었습니다.

- **비용적인 측면**: 저희는 이미 Spring Boot를 통한 백엔드 서버와 API가 별도로 제공되고 있어 DB 접근이 필요하지 않습니다. 따라서 체쿠리는 React의 번들로 배포하는 것이 전체 시스템 구성과 비용 효율성 면에서 더 적합하다고 판단하였습니다.
- **단순성**: 프론트엔드를 정적 파일로 배포함으로써 인프라 구성이 간소화되었으며, Spring Boot와의 역할 분리도 명확해졌습니다.
